...
st.set_page_config(
    page_title="COURTSIDE PRO | Custom Timer",
    page_icon="🏀",
    layout="wide",
...
if 'game_state' not in st.session_state:
    st.session_state.game_state = {
        'team_a': {'name': 'LAKERS', 'score': 0, 'fouls': 0, 'timeouts': 5}, # Set to 5
        'team_b': {'name': 'CELTICS', 'score': 0, 'fouls': 0, 'timeouts': 5}, # Set to 5
        'quarter': 1,
        'time_remaining': 720, # Default 12 mins
        'quarter_length_mins': 12, # Store the preference
        'is_running': False,
        'logs': [],
        'roster_a': [], 
        'roster_b': [],
...
with st.sidebar:
    ...
    # 1. TIMER SETTINGS (NEW)
    st.subheader("⏱ Timer Settings")
    qtr_len = st.number_input("Quarter Duration (Minutes)", min_value=1, max_value=20, value=12)
    ...
    with st.form("add_player"):
        ...
        if st.form_submit_button("Add to Roster"):
            ...
            st.session_state.game_state[t_key].append({
                'name': p_name, 'num': p_num, 'stats': {'pts': 0, 'fouls': 0}
            })
            st.success(f"Added #{p_num}")
    ...
    with c1:
        if st.button("T.O.\n(Home)"):
            if st.session_state.game_state['team_a']['timeouts'] > 0:
                ...
                st.session_state.game_state['logs'].insert(0, {"QTR": st.session_state.game_state['quarter'], "Time": format_clock(st.session_state.game_state['time_remaining']), "Team": st.session_state.game_state['team_a']['name'], "Player": "TEAM", "Event": "TIMEOUT"})
                st.rerun()
        st.caption(f"{st.session_state.game_state['team_a']['timeouts']} Left")

    # Start/Pause & Next QTR
    with c2:
        ...
        if st.button("Next QTR ⏩", use_container_width=True):
            st.session_state.game_state['quarter'] += 1
            # RESET CLOCK BASED ON USER SETTING
    ...
    with c3:
        if st.button("T.O.\n(Away)"):
             if st.session_state.game_state['team_b']['timeouts'] > 0:
                ...
                st.session_state.game_state['logs'].insert(0, {"QTR": st.session_state.game_state['quarter'], "Time": format_clock(st.session_state.game_state['time_remaining']), "Team": st.session_state.game_state['team_b']['name'], "Player": "TEAM", "Event": "TIMEOUT"})
                st.rerun()
...
# >>> COURT ACTIONS <<<
st.divider()
r_a, r_b = st.session_state.game_state['roster_a'], st.session_state.game_state['roster_b']

if len(r_a) < 5 or len(r_b) < 5:
    st.warning("⚠️ Please add at least 5 players to each roster in the Sidebar.")
else:
...
tab_box, tab_qtr, tab_log = st.tabs(["Box Score", "Quarter Analysis", "Full Log"])
...
with tab_box:
    ...
    df_box = pd.concat([build_stats(r_a, st.session_state.game_state['team_a']['name']), build_stats(r_b, st.session_state.game_state['team_b']['name'])], ignore_index=True)
    if not df_box.empty: st.dataframe(df_box, use_container_width=True)

with tab_qtr:
    if st.session_state.game_state['logs']:
        df_l = pd.DataFrame(st.session_state.game_state['logs'])
        if not df_l.empty:
            df_pts = df_l[df_l['Event'].str.contains("PTS", na=False)].copy()
            if not df_pts.empty:
                df_pts['Points'] = df_pts['Event'].str.extract(r'(\d+)').astype(int)
                st.table(df_pts.pivot_table(index='Team', columns='QTR', values='Points', aggfunc='sum', fill_value=0))

with tab_log:
    if st.session_state.game_state['logs']: st.dataframe(pd.DataFrame(st.session_state.game_state['logs']), use_container_width=True)
